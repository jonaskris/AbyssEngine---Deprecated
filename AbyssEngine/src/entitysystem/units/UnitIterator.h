...
namespace abyssengine {
	namespace entitysystem {
		...
		template <typename UnitType>
		class UnitIterator : public UnitIteratorBase
		{
		...
		private:
			...
			explicit UnitIterator(UnitContainer<UnitType>* unitContainer) : unitContainer(unitContainer), currentGroup(std::pair(0, 0)) {};
		public:
			/*
			...
			*/
			...
			/*
				Gets current group.
				Gets all Units in Unit vector if it doesent store Units with OwnedSpecifier.
			*/
			...
			{
				return std::is_base_of<OwnedSpecifier, UnitType>::value;
			}
