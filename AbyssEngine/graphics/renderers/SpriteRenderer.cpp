...
SpriteRenderer::SpriteRenderer() {
	...
	for (size_t i = 0; i < SR_RENDERER_INDICES_SIZE; i += 6)
	{
		int offset = (i / 6) * 4;
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, SR_RENDERER_INDICES_SIZE, IBO_DATA, GL_STATIC_DRAW);
...
void SpriteRenderer::render(const std::vector<GSSComponent*>& components, const mat4& pr_matrix, const mat4& vw_matrix)
{
	...
	while (toRender > 0) {
		...
		if (toRender >= SR_RENDERER_MAX_SPRITES) {
			renderTo = renderFrom + SR_RENDERER_MAX_SPRITES;
		} else {
		...
		for (size_t i = renderFrom; i < renderTo; i++)
		{
			...
			VERTEX_DATA[  i * 4  ].vertex.x = gPosition.x + pPosition.x - (scale.x / 2);
			VERTEX_DATA[  i * 4  ].vertex.y = gPosition.y + pPosition.y - (scale.y / 2);
			VERTEX_DATA[  i * 4  ].vertex.z = gPosition.z + pPosition.z - (scale.z / 2);
			VERTEX_DATA[  i * 4  ].color = color[0];
			VERTEX_DATA[  i * 4  ].uv = uv.at(0);

			VERTEX_DATA[i * 4 + 1].vertex.x = gPosition.x + pPosition.x - (scale.x / 2);
			VERTEX_DATA[i * 4 + 1].vertex.y = gPosition.y + pPosition.y + (scale.y / 2);
			VERTEX_DATA[i * 4 + 1].vertex.z = gPosition.z + pPosition.z - (scale.z / 2);
			VERTEX_DATA[i * 4 + 1].color = color[1];
			VERTEX_DATA[i * 4 + 1].uv = uv.at(1);

			VERTEX_DATA[i * 4 + 2].vertex.x = gPosition.x + pPosition.x + (scale.x / 2);
			VERTEX_DATA[i * 4 + 2].vertex.y = gPosition.y + pPosition.y + (scale.y / 2);
			VERTEX_DATA[i * 4 + 2].vertex.z = gPosition.z + pPosition.z - (scale.z / 2);
			VERTEX_DATA[i * 4 + 2].color = color[2];
			VERTEX_DATA[i * 4 + 2].uv = uv.at(2);

			VERTEX_DATA[i * 4 + 3].vertex.x = gPosition.x + pPosition.x + (scale.x / 2);
			VERTEX_DATA[i * 4 + 3].vertex.y = gPosition.y + pPosition.y - (scale.y / 2);
			VERTEX_DATA[i * 4 + 3].vertex.z = gPosition.z + pPosition.z - (scale.z / 2);
			VERTEX_DATA[i * 4 + 3].color = color[3];
			VERTEX_DATA[i * 4 + 3].uv = uv.at(3);
		}

		glBufferData(GL_ARRAY_BUFFER, SR_RENDERER_SPRITE_SIZE * (renderTo - renderFrom), VERTEX_DATA, GL_STATIC_DRAW);
